...
Findings of chi-square test: p-value (< 2.2e-16)
...
var.test(cc_weight, og_weight)
```
This F-test indicates that there is a significant difference of biomass between clear cut and old growth. Because the confidence interval doesn't have 1, it means the variances are different. Because the p-value = (1.439e-07) is very small is suggests that there is statistic significance.
...
ggarrange(gghistogram(trout_clean$length_2_mm, title = "Length"),
...
```{r}
ggarrange(
 gghistogram(log(trout_clean$length_2_mm), title = "Length"), 
 gghistogram(log(trout_clean$weight_g), title = "Weight") )
```

```{r}
cor.test(log(trout_clean$length_2_mm), log(trout_clean$weight_g))
```
This small p-value (<2.2e-16) and the high correlation coefficient means that there is a positive correlation between body length and body mass. 
...
```{r}
pie_crab |> 
  ggboxplot(x = 'latitude', y = 'size', col = 'site') +
  geom_jitter(size =.25) + 
  theme(legend.postition = "none")
```

2) This test shows that as latitude increases, the size of crabs increase. 
...
summary(pie_lm)
```
This p-value tells us that we fail to reject the null hypothesis and that there is not a strong effect of water temperature on carapace width. 
...
pie_crab |> 
  ...
  cor()
```
This multiple linear regression model shows that latitude and air temperature are strongly correlated, while latitude and water temperature are not. 
